...
public class LogUtils {
    ...
    /**
        MDC = 日志的临时标签本
        比喻：要排查 用户1001 的问题，希望所有日志都带上 用户1001。
        手动写：每次日志都拼 用户1001，麻烦又容易错；
        MDC：把标签放进上下文，这段代码的所有日志自动带标签，用完清空。
        代码逻辑：
        MDC.put(键, 值) → 放标签 → 打印日志 → MDC.clear() → 清空标签。
     */
     ...
     * @param path API路径（/api/user/1001等）
     * @param userId 用户ID
     * @param statusCode 状态码（200、404等）
     * @param duration 耗时（毫秒）
     */
     ...
     * 记录文件上传、下载日志
     * @param userId 用户ID
     * @param operation 操作名称（UPLOAD、DOWNLOAD等）
     * @param fileName 文件名
     ...
     * @param messageType 消息类型（TEXT、IMAGE等）
     * @param messageLength 消息长度（字符数）
     */
     ...
     * @param status 状态（STARTED、FAILED等）
     * @param details 详细信息
     */
     ...
     * 格式化消息
     * @param message 消息模板
     * @param args 格式化参数,可变参数(任意多个，类型随便)
     * @return 格式化后的消息
     */
    private static String formatMessage(String message, Object... args) {
        ...
        try {
            // 把 args 里的参数，挨个填进 message 的占位符里。
