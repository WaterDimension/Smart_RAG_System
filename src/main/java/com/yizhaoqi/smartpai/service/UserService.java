...
public class UserService {
    ...
    //工厂模式：日志输出的前缀就会自动带上UserService全类名
     ...
     * @throws CustomException 如果注册策略不允许注册
     */
    private void validateRegistrationPolicy(String username, String inviteCode) {
        ...
        // 如果是 INVITE_ONLY 模式，且没有提供邀请码，禁止注册
        if (inviteRequired) {
    ...
    private void ensureDefaultOrgTagExists() {
        ...
        if (!organizationTagRepository.existsByTagId(DEFAULT_ORG_TAG)) {
            ...
            // 从数据库查出所有用户 → 只留下【管理员】 → 取出第一个管理员
            Optional<User> adminUser = userRepository.findAll().stream()
                    .filter(user -> User.Role.ADMIN.equals(user.getRole()))  //java8 新增的流操作
                    .findFirst();
            //Optional容易包装类，Java 8 专门防止空指针异常的「安全容器类」
            // Optional 自带的orElseThrow替代老式的 if (xxx == null) 判断如果没有管理员，orElseThrow抛异常（系统初始化失败）
